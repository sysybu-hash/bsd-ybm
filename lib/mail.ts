...
export async function sendWelcomeEmail(toEmail: string, name: string | null): Promise<void> {
  ...
  const inner = `
    <h1 style="margin:0 0 16px;font-size:22px;font-weight:800;color:#f8fafc;text-align:center;">ברוכים הבאים ל־BSD-YBM</h1>
...
export async function sendSubscriptionTierInvitationEmail(
  ...
  const r = await sendTransactionalEmail(
    toEmail.trim().toLowerCase(),
    `BSD-YBM — הזמנת מנוי (${params.tierLabel})`,
    inner,
  );
  if (!r.ok) return { ok: false, error: r.error };
  return { ok: true };
