...
async function main() {
  await sharp(Buffer.from(iconSvg(192))).png().toFile(join(publicDir, "icon-192.png"));
  await sharp(Buffer.from(iconSvg(512))).png().toFile(join(publicDir, "icon-512.png"));
  await sharp(Buffer.from(ogSvg())).png().toFile(join(publicDir, "og-image.png"));
  console.log("OK: public/icon-192.png, icon-512.png, og-image.png");
}
