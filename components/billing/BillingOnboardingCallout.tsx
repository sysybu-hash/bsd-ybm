...
export default function BillingOnboardingCallout({ text }: { text: string }) {
  ...
  return (
    <div
      className="rounded-2xl border border-emerald-200/90 bg-gradient-to-br from-emerald-50/90 to-sky-50/50 p-5 sm:p-6 text-sm text-slate-800 shadow-sm"
      dir="rtl"
    >
      ...
      <div className="rounded-xl bg-white/90 border border-slate-100 p-4 text-slate-800 leading-relaxed whitespace-pre-wrap text-[13px]">
        {text}
      </div>
      <button
        type="button"
        onClick={() => void copy()}
        className="mt-3 inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50"
      >
