...
export default function SubscriptionPricingTable({ tierPricesIls }: Props) {
  ...
  return (
    <section className="card-avenue bg-[#0a0b14] p-6 shadow-sm md:p-10" dir={dir}>
      <div className="mb-10 text-center">
        ...
        <p className="mx-auto mt-2 max-w-2xl text-sm font-medium text-white/45">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5 lg:grid-cols-5">
        {ORDER.map((tier) => {
          ...
          const cardBase = highlight
            ? "border-indigo-500/30 bg-[#0a0b14] shadow-sm ring-1 ring-indigo-100"
            : "border-white/[0.08]/90 bg-[#0a0b14] shadow-sm hover:border-indigo-500/30/60 hover:shadow-md"
          return (
            <article
              key={tier}
            ...
            >
              ...
              <p className="mb-4 text-[10px] font-bold uppercase tracking-wider text-white/35">{tier}</p>
              <div className="mb-4">
                {price != null ? (
                  <p className="text-3xl font-black tabular-nums text-white">
                    ₪{price.toFixed(1)}
                    <span className="text-sm font-semibold text-white/45"> /חודש</span>
                  </p>
                ) : (
                  <p className="text-lg font-black text-white/65">צור קשר</p>
                )}
              </div>
              <table className="mb-4 w-full flex-1 border-collapse text-sm text-white/55">
                <tbody>
                  <tr className="border-b border-white/[0.08]/80">
                  ...
                  <tr className="border-b border-white/[0.08]/80">
                    <td className="py-2.5 pe-2 ps-0 font-semibold text-white/65">פרימיום (Pro)</td>
                    <td className="py-2.5 text-end font-black tabular-nums text-white">{a.premiumScans}</td>
                  </tr>
                  <tr>
                    <td className="py-2.5 pe-2 ps-0 font-semibold text-white/45">חברות</td>
