  ...
  return (
    <div className="relative z-[2]" dir={dir}>
      <motion.button
        ...
        className="flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-blue-600 to-indigo-600 text-white shadow-xl shadow-blue-600/35 ring-2 ring-white/90"
      >
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <div
              className={`overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-2xl ${
                tab === "chat" ? "" : ""
              }`}
            >
              <div
                ...
                <div className="flex shrink-0 gap-1 rounded-xl bg-black/15 p-0.5">
                  <button
                    ...
                    className={`rounded-lg px-2 py-1 text-[10px] font-black uppercase ${
                      tab === "chat" ? "bg-white text-slate-900" : "text-white/80 hover:bg-white/10"
                    }`}
                  >
                    צ׳אט
                  </button>
                  <button
                    ...
                    className={`rounded-lg px-2 py-1 text-[10px] font-black uppercase ${
                      tab === "finance" ? "bg-white text-slate-900" : "text-white/80 hover:bg-white/10"
                    }`}
                  >
                    כספים
                  </button>
              ...
              {tab === "chat" ? (
                <>
                  <div
                    ref={scrollRef}
                    className="h-56 space-y-2 overflow-y-auto bg-slate-50 p-3 text-sm"
                  >
                    {messages.map((m, i) => (
                      <div
                  ...
                  <div className="flex gap-2 border-t border-slate-200 bg-white p-2">
                    <input
                      ...
                      className="min-w-0 flex-1 rounded-xl border border-slate-200 px-3 py-2 text-xs text-slate-900 outline-none focus:border-blue-500"
                    />
                    <button
                      type="button"
                      disabled={sending}
                      onClick={() => void sendChat()}
                      className="rounded-xl bg-blue-600 p-2 text-white disabled:opacity-50"
                ...
                <div className="bg-white">
                  <div className="flex items-center gap-2 border-b border-slate-100 bg-blue-50/50 px-3 py-2 text-xs font-bold text-blue-900">
                    <Bot size={14} aria-hidden />
                    שאלות על הוצאות, הכנסות ומסמכים
                  </div>
                  <div className="h-44 overflow-y-auto p-3 text-xs text-slate-700">
                    {fAnswer ? (
                    ...
                    {fLoading ? (
                      <Loader2 className="mt-2 animate-spin text-blue-600" size={18} />
                    ) : null}
                  </div>
