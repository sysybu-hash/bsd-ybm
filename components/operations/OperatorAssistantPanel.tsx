    ...
    <div className="space-y-5" dir="rtl">
      <section className="overflow-hidden rounded-[32px] border border-white/[0.08] bg-[#0a0b14] shadow-sm">
        <div className="bg-[linear-gradient(135deg,_#f8fbff_0%,_#eef6ff_55%,_#ffffff_100%)] px-6 py-7 md:px-8">
          <p className="inline-flex items-center gap-2 rounded-full border border-indigo-500/20 bg-[#0a0b14] px-3 py-1 text-xs font-black text-indigo-300">
            <Bot size={13} />
            Operator Agent
          </p>
          <h1 className="mt-3 text-3xl font-black tracking-tight text-white">עוזר תפעולי</h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-white/55">מסך אחד לשאלה אחת בכל פעם. בלי לוח עמוס, בלי קיצורי דרך מיותרים, רק תשובה תפעולית מהירה ובטוחה.</p>

          <div className="mt-6 grid gap-3 md:grid-cols-2">
            <div className="rounded-[28px] border border-white/[0.08] bg-[#0a0b14] p-4">
      ...
      <section className="rounded-[28px] border border-white/[0.08] bg-[#0a0b14] p-4 shadow-sm">
        <div className="mb-3 flex items-center justify-between gap-3">
          <h2 className="text-sm font-black text-white">שיחה</h2>
          <span className="rounded-full bg-white/[0.05] px-2.5 py-1 text-xs font-bold text-white/55">מצב בטוח</span>
        </div>
        <div className="max-h-[46vh] space-y-2 overflow-y-auto rounded-[24px] border border-white/[0.08] bg-white/[0.03] p-3.5">
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.role === "user" ? "justify-start" : "justify-end"}`}>
              <div className={`max-w-[88%] rounded-[22px] px-3.5 py-2.5 text-sm leading-6 ${m.role === "user" ? "bg-gray-900 text-white" : "border border-white/[0.08] bg-[#0a0b14] text-white/75"}`}>
                {m.text}
              </div>
            </div>
          ))}
          {loading ? (
            <div className="flex justify-end text-white/45"><Loader2 size={16} className="animate-spin" /></div>
          ) : null}
        </div>
        ...
        <div className="mt-3 flex gap-2">
          <input
            ...
            className="min-w-0 flex-1 rounded-2xl border border-white/[0.08] bg-white/[0.03] px-4 py-3 text-sm outline-none transition focus:border-indigo-400 focus:bg-[#0a0b14] focus:ring-2 focus:ring-indigo-500/15"
          />
          <button
            type="button"
            onClick={() => void send(false)}
            disabled={loading || !input.trim()}
            className="rounded-2xl bg-gray-900 px-4 py-3 text-white disabled:opacity-50"
          >
            <Send size={15} />
          </button>
        </div>
...
function QuickBtn({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="rounded-xl border border-gray-300 bg-[#0a0b14] px-3 py-1.5 text-xs font-bold text-white/65 hover:bg-white/[0.03]"
    >
      {label}
    </button>
  );
