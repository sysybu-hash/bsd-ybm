    ...
    <div className="space-y-5" dir="rtl">
      <section className="overflow-hidden rounded-[32px] border border-gray-200 bg-white shadow-sm">
        <div className="bg-[linear-gradient(135deg,_#f8fbff_0%,_#eef6ff_55%,_#ffffff_100%)] px-6 py-7 md:px-8">
          <p className="inline-flex items-center gap-2 rounded-full border border-indigo-100 bg-white px-3 py-1 text-xs font-black text-indigo-700">
            <Bot size={13} />
            Operator Agent
          </p>
          <h1 className="mt-3 text-3xl font-black tracking-tight text-gray-900">עוזר תפעולי</h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-gray-600">מסך אחד לשאלה אחת בכל פעם. בלי לוח עמוס, בלי קיצורי דרך מיותרים, רק תשובה תפעולית מהירה ובטוחה.</p>
      ...
      <section className="rounded-[28px] border border-gray-200 bg-white p-4 shadow-sm">
        <div className="mb-3 flex items-center justify-between gap-3">
          ...
          <span className="rounded-full bg-gray-100 px-2.5 py-1 text-xs font-bold text-gray-600">מצב בטוח</span>
        </div>
        <div className="max-h-[46vh] space-y-2 overflow-y-auto rounded-[24px] border border-gray-200 bg-gray-50 p-3.5">
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.role === "user" ? "justify-start" : "justify-end"}`}>
              <div className={`max-w-[88%] rounded-[22px] px-3.5 py-2.5 text-sm leading-6 ${m.role === "user" ? "bg-gray-900 text-white" : "border border-gray-200 bg-white text-gray-800"}`}>
                {m.text}
              </div>
            </div>
          ))}
          {loading ? (
            <div className="flex justify-end text-gray-500"><Loader2 size={16} className="animate-spin" /></div>
          ) : null}
        </div>
...
function QuickBtn({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="rounded-xl border border-gray-300 bg-white px-3 py-1.5 text-xs font-bold text-gray-700 hover:bg-gray-50"
    >
      {label}
    </button>
  );
