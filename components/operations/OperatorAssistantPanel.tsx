    ...
    <div className="space-y-5" dir="rtl">
      <section className="overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-sm">
        <div className="bg-[linear-gradient(135deg,_#f8fbff_0%,_#eef6ff_55%,_#ffffff_100%)] px-6 py-7 md:px-8">
          <p className="inline-flex items-center gap-2 rounded-full border border-blue-100 bg-white px-3 py-1 text-xs font-black text-blue-700">
            <Bot size={13} />
            Operator Agent
          </p>
          <h1 className="mt-3 text-3xl font-black tracking-tight text-slate-900">עוזר תפעולי</h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">מסך אחד לשאלה אחת בכל פעם. בלי לוח עמוס, בלי קיצורי דרך מיותרים, רק תשובה תפעולית מהירה ובטוחה.</p>

          <div className="mt-6 grid gap-3 md:grid-cols-2">
            <div className="rounded-[28px] border border-slate-200 bg-white p-4">
              <p className="text-sm font-black text-slate-900">שאלות מהירות</p>
      ...
      <section className="rounded-[28px] border border-slate-200 bg-white p-4 shadow-sm">
        <div className="mb-3 flex items-center justify-between gap-3">
          <h2 className="text-sm font-black text-slate-900">שיחה</h2>
          <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-600">מצב בטוח</span>
        </div>
        <div className="max-h-[46vh] space-y-2 overflow-y-auto rounded-[24px] border border-slate-200 bg-slate-50 p-3.5">
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.role === "user" ? "justify-start" : "justify-end"}`}>
              <div className={`max-w-[88%] rounded-[22px] px-3.5 py-2.5 text-sm leading-6 ${m.role === "user" ? "bg-slate-900 text-white" : "border border-slate-200 bg-white text-slate-800"}`}>
                {m.text}
              </div>
            </div>
          ))}
        ...
        <div className="mt-3 flex gap-2">
          <input
            ...
            className="min-w-0 flex-1 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none transition focus:border-blue-400 focus:bg-white focus:ring-2 focus:ring-blue-500/15"
          />
          <button
            type="button"
        ...
        {needsConfirm ? (
          ...
          <div className="mt-3 inline-flex items-center gap-1.5 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs text-emerald-800">
