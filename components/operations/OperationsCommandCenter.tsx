  ...
  return (
    <div className="space-y-6" dir="rtl">
      <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div className="relative border-b border-gray-100 bg-gradient-to-br from-teal-50 via-white to-cyan-50 px-6 py-7 md:px-8">
          <div className="absolute inset-y-0 start-0 w-1.5 bg-teal-600" />
          <div className="relative flex flex-wrap items-start justify-between gap-4">
            <div className="max-w-2xl">
              <span className="inline-flex items-center gap-1.5 rounded-full border border-teal-200 bg-teal-50 px-3 py-1 text-[11px] font-bold text-teal-700">
                <ListChecks size={11} />
                מרכז תפעול
              </span>
              <h1 className="mt-3 text-2xl font-black tracking-tight text-gray-900">תמונה תפעולית אחת, בלי עומס מייתר</h1>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Link href="/app/intelligence" className="rounded-xl bg-teal-600 px-4 py-2 text-sm font-bold text-white shadow-sm transition-colors hover:bg-teal-700">
                פתח עוזר תפעולי
              </Link>
            </div>
          </div>
        ...
        <div className="grid gap-5 px-6 py-6 md:px-8 xl:grid-cols-[1.15fr_0.85fr]">
          <section className="space-y-4">
            <Panel
              ...
              icon={<Gauge size={17} className="text-teal-400" />}
            >
            ...
            <Panel
              ...
              icon={<Workflow size={17} className="text-teal-400" />}
            >
          ...
          <section className="space-y-4">
            ...
            <Panel
              ...
              icon={<Bell size={17} className="text-teal-400" />}
            >
              <div className="space-y-2">
                {data.recentActivity.length === 0 ? (
                  ...
                  data.recentActivity.map((activity, index) => (
                    <div key={`${activity.action}-${index}`} className="rounded-xl border border-gray-200 bg-white px-4 py-3">
  ...
  return (
    <article className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
      <div className="flex items-start gap-3">
        <div className="mt-0.5 flex h-9 w-9 items-center justify-center rounded-xl bg-teal-50 border border-teal-100">{icon}</div>
        <div>
...
function ToggleCard({
  ...
  return (
    ...
    >
      ...
      <span className={`rounded-full px-3 py-1 text-xs font-black ${on ? "bg-emerald-100 text-emerald-700" : "bg-gray-100 text-gray-400"}`}>
        {on ? "פעיל" : "כבוי"}
      </span>
    </button>
  );
