...
function TutorialVideoOrAnimation({ kind }: { kind: Kind }) {
  ...
  return (
    <div className="relative aspect-video w-full overflow-hidden rounded-[1.75rem] border border-slate-200 bg-slate-100 shadow-inner">
      {mode !== "anim" ? (
        <video
          className={
            mode === "video"
      ...
      <div className="pointer-events-none absolute end-3 top-3 rounded-full border border-slate-200/80 bg-white/95 px-2 py-0.5 text-[10px] font-black uppercase tracking-wider text-slate-600 shadow-sm backdrop-blur-sm">
...
function AnimCrm() {
  return (
    <div className="flex h-full flex-col justify-center gap-3 text-slate-800">
      <div className="mb-1 flex items-center gap-2 text-xs font-bold text-blue-700">
        <Users size={16} /> CRM
      </div>
      <div className="bsd-tut-crm-row rounded-xl bg-white px-3 py-2.5 shadow-sm ring-1 ring-slate-200/80">
        <div className="h-2 w-1/3 rounded bg-blue-200" />
        <div className="mt-2 h-2 w-2/3 rounded bg-slate-200" />
      </div>
      <div className="bsd-tut-crm-row rounded-xl bg-white px-3 py-2.5 shadow-sm ring-1 ring-slate-200/80">
        <div className="h-2 w-2/5 rounded bg-blue-200" />
...
function AnimScanner() {
  return (
    <div className="relative flex h-full flex-col items-center justify-center text-slate-800">
      <div className="mb-2 flex items-center gap-2 text-xs font-bold text-violet-700">
        <ScanLine size={16} /> Scan
      </div>
      <div className="bsd-tut-scan-box relative flex h-[62%] w-[78%] items-center justify-center rounded-2xl border-2 border-dashed border-violet-300 bg-violet-50/90">
        ...
        <div className="bsd-tut-scan-file rounded-lg bg-white px-4 py-3 shadow-md ring-1 ring-slate-200/80">
          <div className="h-2 w-16 rounded bg-violet-200" />
          <div className="mt-2 h-2 w-12 rounded bg-slate-200" />
        </div>
      </div>
      <div className="mt-3 h-1.5 w-[70%] overflow-hidden rounded-full bg-slate-200">
        <div className="bsd-tut-scan-progress h-full rounded-full bg-violet-600" />
      </div>
    </div>
  );
}
...
export default function LandingTutorialSection() {
  ...
  return (
    <section
      id="tutorial-videos"
      className="relative z-20 mx-4 mt-2 mb-24 scroll-mt-6 rounded-[4rem] border border-slate-200 bg-white p-12 shadow-2xl shadow-slate-200/50 md:mx-10 md:p-20"
      dir={dir}
    >
      <div className="mx-auto max-w-6xl text-center">
        <span className="inline-flex items-center gap-2 rounded-full bg-blue-100 px-4 py-1.5 text-[10px] font-black uppercase tracking-widest text-blue-800">
          <Play size={12} className="shrink-0" aria-hidden />
          {t("landing.tutorialSectionKicker")}
        </span>
        <h2 className="mt-4 text-3xl font-black italic tracking-tight text-slate-900 md:text-5xl">
      ...
      <div className="mx-auto mt-14 grid max-w-6xl grid-cols-1 gap-10 md:grid-cols-3 md:gap-8">
        {items.map(({ kind, titleKey, bodyKey }) => {
          ...
          return (
            <article
              key={kind}
              className="flex flex-col rounded-[2rem] border border-slate-100 bg-slate-50/80 p-6 shadow-md md:p-7"
            >
              <div className="mb-4 flex items-center justify-center gap-2 text-blue-700 md:justify-start">
                <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-100">
                  <Icon size={20} aria-hidden />
                </span>
              </div>
              <TutorialVideoOrAnimation kind={kind} />
              <h3 className="mt-5 text-lg font-black text-slate-900 md:text-xl">{t(titleKey)}</h3>
