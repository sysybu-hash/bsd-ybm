...
## Lighthouse (baseline מקומי)

סקריפט אופציונלי `npm run lighthouse:sample` מריץ Lighthouse על בסיס ברירת המחדל `http://127.0.0.1:3000` בנתיבים ציבוריים `/`, `/login`, `/product`. יש להפעיל קודם `npm run dev` ברקע. הפלט הוא ציוני קטגוריות (ביצועים, נגישות, שיטות מומלצות, SEO); המספרים משתנים לפי תוכן, רשת ומכונה ואינם מהווים הבטחת "ציון מושלם". ברירת המחדל יוצאת עם קוד יציאה 0; `--fail-on-budget` מפעיל ספים פנימיים ויכול להיכשל אם יוגדר במפורש. בסיס אחר: ארגומנט `--base=<url>` או משתנה סביבה `LIGHTHOUSE_BASE_URL`.
...
קיימים מסלולי `/dashboard/*` (הפניות ב־middleware/`next.config`) לצד `/app/*`. עץ `app/dashboard` הוסר מהקוד; אין דפי Next תחת `/dashboard`. מדיניות מומלצת:
