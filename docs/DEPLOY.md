...
מיגרציה **`20260319190000_sync_run`** יוצרת טבלת **`SyncRun`** — לוג אוטומטי מ־`POST /api/sync` (סטטוס DB, מקאנו, pipeline). בלי מיגרציה זו השמירה נכשלת בשקט וה-API ממשיך לעבוד.
