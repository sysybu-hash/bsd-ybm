  ...
  return (
    <div className="min-h-screen bg-slate-50" dir="rtl">
      ...
      <main className="max-w-4xl mx-auto px-6 py-24 pt-32 pb-16">
        <h1 className="text-4xl font-black italic text-slate-900 mb-3">מסמכים משפטיים</h1>
        ...
        <ul className="grid gap-4 md:grid-cols-1">
          {items.map(({ href, title, desc, icon: Icon }) => (
            <li key={href}>
              <Link
                href={href}
                className="flex gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:border-blue-200 hover:shadow-md"
              >
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-[var(--primary-color)]">
                  <Icon size={24} />
                </div>
                <div>
