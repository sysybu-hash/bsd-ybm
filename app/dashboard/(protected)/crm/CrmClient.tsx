...
const DOC_STATUS_LABEL: Record<string, { label: string; cls: string }> = {
  PENDING: { label: "����� ������", cls: "bg-amber-500/[0.15] text-amber-700" },
  PAID:    { label: "����",         cls: "bg-emerald-500/[0.12] text-emerald-700" },
  CANCELLED: { label: "�����",      cls: "bg-gray-50 text-gray-400" },
};
...
const STATUS_COLUMNS = [
  { key: "LEAD",        label: "���",     bg: "bg-indigo-50",   border: "border-indigo-200",  badge: "bg-indigo-50 text-indigo-700 border-indigo-200",   dot: "bg-indigo-500" },
  { key: "ACTIVE",      label: "����",    bg: "bg-sky-50",     border: "border-sky-200",     badge: "bg-sky-50 text-sky-700 border-sky-200",           dot: "bg-sky-500"    },
  { key: "PROPOSAL",    label: "����",    bg: "bg-violet-50",  border: "border-violet-200",  badge: "bg-violet-50 text-violet-700 border-violet-200",  dot: "bg-violet-500" },
  { key: "CLOSED_WON",  label: "���� ?",  bg: "bg-emerald-50", border: "border-emerald-200", badge: "bg-emerald-50 text-emerald-700 border-emerald-200", dot: "bg-emerald-500" },
  { key: "CLOSED_LOST", label: "���� ?",  bg: "bg-rose-50",    border: "border-rose-200",    badge: "bg-rose-50 text-rose-700 border-rose-200",       dot: "bg-rose-500"   },
] as const;
      ...
      <div className="relative w-full max-w-lg overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl shadow-black/50">
        <div className="flex items-center justify-between border-b border-gray-100 px-6 py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-black text-gray-900">{isEdit ? "����� ����" : "���� ���"}</p>
              {isEdit && c && <p className="text-xs text-gray-400">���� {fmtDate(c.createdAt)}</p>}
            </div>
          </div>
          <button type="button" onClick={onClose} className="rounded-xl p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-700 transition">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
          {err && (
            <div className="rounded-xl border border-rose-500/20 bg-rose-500/[0.08] px-3 py-2.5 text-sm text-rose-700">{err}</div>
          )}
          <div>
            <label className="block text-xs font-bold text-gray-400 mb-1">�� *</label>
            <input value={name} onChange={e => setName(e.target.value)} placeholder="�� ���� / ����" className={inputCls} autoFocus />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <div className="relative">
                <select value={status} onChange={e => setStatus(e.target.value as StatusKey)} className={inputCls + " appearance-none bg-white"}>
          ...
          <div>
            ...
            <div className="relative">
              <select value={projectId} onChange={e => setProjectId(e.target.value)} className={inputCls + " appearance-none bg-white"}>
                <option value="">� ��� ������ �</option>
                {projects.map(p => <option key={p.id} value={p.id}>{p.name}{!p.isActive ? " (������)" : ""}</option>)}
              </select>
              <ChevronDown size={13} className="pointer-events-none absolute end-3 top-1/2 -translate-y-1/2 text-gray-400" />
            </div>
          </div>
          <div>
            <label className="block text-xs font-bold text-gray-400 mb-1">�����</label>
            <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="����� �������..." rows={3} className={inputCls + " resize-none"} />
          </div>
            ...
            <div className="rounded-2xl border border-indigo-500/15 bg-indigo-500/[0.06] p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <Link
                  ...
                  className="text-[10px] font-black text-indigo-400 hover:text-indigo-700 flex items-center gap-1"
                >
                  + ���� ����
                </Link>
              </div>
              ...
              {invoices.length === 0 ? (
                ...
                <>
                  ...
                  <div className="grid grid-cols-3 gap-2 border-b border-gray-100 pb-3">
                    ...
                    <div className="text-center">
                      <p className="text-[10px] text-gray-400 mb-0.5">����</p>
                      <p className="text-sm font-black text-amber-700">{fmtMoney(invoices.filter(i => i.status === "PENDING").reduce((s, i) => s + i.total, 0))}</p>
                    </div>
                  </div>
                  {/* ����� �������� */}
                  <div className="space-y-1.5 max-h-[200px] overflow-y-auto">
                    {invoices.map(inv => {
                      ...
                      return (
                        <div key={inv.id} className="flex items-center gap-2 rounded-xl border border-gray-100 bg-gray-50 px-3 py-2">
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              <span className="text-xs font-black text-gray-900">{fmtMoney(inv.total)}</span>
                          ...
                          {inv.status === "PENDING" && (
                            <div className="flex gap-1 shrink-0">
                              <button
                                ...
                                className="rounded-lg bg-emerald-500/[0.12] px-2 py-1 text-[9px] font-black text-emerald-700 hover:bg-emerald-500/20 transition"
                              >
                                ����
                              </button>
                              <button
                                ...
                                className="rounded-lg bg-gray-50 px-2 py-1 text-[9px] font-black text-gray-400 hover:bg-gray-100 transition"
                              >
                                ���
                              </button>
                            </div>
        ...
        <div className="flex items-center justify-between border-t border-gray-100 px-6 py-4 bg-gray-50">
          <div className="flex items-center gap-2">
            ...
            {isEdit && c?.status === "CLOSED_WON" && (
              <Link
                ...
                className="flex items-center gap-1.5 rounded-xl border border-indigo-500/20 bg-indigo-500/[0.08] px-3 py-2 text-xs font-bold text-indigo-700 hover:bg-indigo-500/15 transition"
              >
                <ReceiptText size={13} /> ���� �������
              </Link>
            )}
          ...
          <div className="flex items-center gap-2">
            <button type="button" onClick={onClose} className="rounded-xl border border-gray-200 bg-gray-50 px-4 py-2 text-sm font-bold text-gray-600 hover:bg-gray-100 transition">
              �����
            </button>
...
function ContactCard({
  ...
  return (
    <div
      className="group relative rounded-2xl border border-gray-100 bg-white p-4 hover:border-gray-200 hover:bg-gray-50 transition-all cursor-pointer"
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        {contact.value != null && (
          <span className="text-xs font-black text-emerald-700 bg-emerald-500/[0.10] rounded-lg px-2 py-0.5">
      ...
      {(contact.erp?.invoiceCount ?? 0) > 0 && (
        <div className="mt-2 flex items-center gap-1.5 text-[10px] font-black text-indigo-700">
          <ReceiptText size={10} />
          <span>{contact.erp!.invoiceCount} ��������</span>
          {contact.erp!.totalPending > 0 && (
            <span className="rounded-full bg-amber-500/[0.12] text-amber-700 px-1.5 py-0.5">����: {fmtMoney(contact.erp!.totalPending)}</span>
          )}
          {contact.erp!.totalPending === 0 && contact.erp!.totalPaid > 0 && (
            <span className="rounded-full bg-emerald-500/[0.10] text-emerald-700 px-1.5 py-0.5">���� ?</span>
          )}
        </div>
      )}
      ...
      {(contact.email || contact.phone) && (
        <div className="mt-2.5 space-y-1">
          {contact.phone && (
            <div className="flex items-center gap-1.5 text-[10px] text-gray-400" dir="ltr">
              <Phone size={10} className="shrink-0" /> {contact.phone}
            </div>
          )}
          {contact.email && (
            <div className="flex items-center gap-1.5 text-[10px] text-gray-400 truncate" dir="ltr">
      ...
      {contact.status === "CLOSED_WON" && (
        <Link
          ...
          className="mt-2 flex items-center gap-1 text-[10px] font-bold text-indigo-400 hover:text-indigo-700 transition"
        >
          <ReceiptText size={10} /> ���� �������
        </Link>
      )}
      ...
      {menuOpen && (
        <div
          className="absolute left-0 top-full z-20 mt-1 min-w-[140px] rounded-2xl border border-gray-200 bg-white shadow-lg shadow-black/40 py-1.5"
          onClick={(e) => e.stopPropagation()}
        >
          <p className="px-3 py-1 text-[10px] font-bold text-gray-400 uppercase tracking-wider">��� �����</p>
          {STATUS_COLUMNS.map(s => (
            <button
              key={s.key}
              type="button"
              onClick={() => { onStatusChange(contact.id, s.key); setMenuOpen(false); }}
              className={`flex w-full items-center gap-2 px-3 py-1.5 text-xs font-bold hover:bg-gray-100 transition ${contact.status === s.key ? "text-indigo-700 bg-indigo-500/[0.08]" : "text-gray-600"}`}
            >
          ...
          <div className="border-t border-gray-100 mt-1 pt-1">
            <button
              ...
              className="flex w-full items-center gap-2 px-3 py-1.5 text-xs font-bold text-gray-600 hover:bg-gray-100"
            >
              <Edit3 size={12} /> ����
            </button>
          </div>
...
export default function CrmClient({
  ...
  const saveProject = async () => {
    if (!projName.trim()) { setProjErr("�� ����� ��"); return; }
      ...
      <div className="p-6 space-y-8" dir={dir}>
        ...
        {organizations.length > 0 && (
          <section className="space-y-4">
            <h2 className="flex items-center gap-2 text-xl font-black text-gray-900">`n              <LayoutGrid className="text-indigo-600" size={20} /> ������� ������
            </h2>
    ...
    <div className="flex flex-col gap-5 min-h-screen" dir="rtl">
      ...
      <section className="relative overflow-hidden rounded-2xl border border-indigo-500/20 bg-white px-6 py-5 md:px-8">
        ...
        <div className="max-w-[1400px] mx-auto flex items-center justify-between gap-4 flex-wrap">
          <div>
            <span className="inline-flex items-center gap-1.5 rounded-full border border-indigo-500/20 bg-indigo-500/[0.10] px-3 py-1 text-[11px] font-bold text-indigo-700">
              <Users size={11} /> ����� ������
            </span>
            <h1 className="mt-2 text-xl font-black text-gray-900">CRM � ���� ������</h1>
            <p className="text-xs text-gray-400 mt-0.5">������, �����, �������� ������ ����</p>
          </div>
          <div className="flex items-center gap-2">
            <div className="inline-flex rounded-xl border border-gray-200 bg-gray-50 p-0.5">
              <button type="button" onClick={() => setView("pipeline")} className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-bold transition ${view === "pipeline" ? "bg-indigo-600 text-white shadow-sm" : "text-gray-400 hover:text-gray-700"}`}>
                <BarChart2 size={13} /> ��������
              </button>
              <button type="button" onClick={() => setView("list")} className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-bold transition ${view === "list" ? "bg-indigo-600 text-white shadow-sm" : "text-gray-400 hover:text-gray-700"}`}>
                <List size={13} /> �����
              </button>
              <button type="button" onClick={() => setView("projects")} className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-bold transition ${view === "projects" ? "bg-indigo-600 text-white shadow-sm" : "text-gray-400 hover:text-gray-700"}`}>
                <Briefcase size={13} /> ��������
              </button>
            </div>
            <button
      ...
      <div className="flex-1 overflow-auto px-0 py-0">
        <div className="max-w-[1400px] mx-auto space-y-5">
          ...
          {/* ?? Toast ?? */}
          {msg && (
            <div className={`flex items-center gap-3 rounded-2xl px-4 py-3 text-sm ${msg.startsWith("?") ? "border border-emerald-500/20 bg-emerald-500/[0.08] text-emerald-700" : "border border-rose-500/20 bg-rose-500/[0.08] text-rose-700"}`}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="rounded-2xl border border-gray-100 bg-white p-4">
              ...
              <p className="text-[10px] text-gray-400 mt-0.5">{contacts.filter(c => c.status === "LEAD").length} �����</p>
            </div>
            <div className="rounded-2xl border border-indigo-500/15 bg-indigo-500/[0.06] p-4">
              <div className="flex items-center gap-2 mb-1">
                <TrendingUp size={14} className="text-indigo-400" />
                <p className="text-xs text-indigo-700 font-bold">�������� ����</p>
              </div>
            ...
            <div className="rounded-2xl border border-emerald-500/15 bg-emerald-500/[0.06] p-4">
              <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-xs text-emerald-700 font-bold">����� ������</p>
              </div>
            ...
            <div className="rounded-2xl border border-sky-500/15 bg-sky-500/[0.06] p-4">
              <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-xs text-sky-700 font-bold">���� �����</p>
              </div>
              <p className="text-2xl font-black text-sky-700">{winRate != null ? `${winRate}%` : "�"}</p>
              <p className="text-[10px] text-sky-400 mt-0.5">{closedAll} ������ ������</p>
            </div>
          </div>
                  ...
                  <div key={col.key} className="flex flex-col min-w-[220px] w-[220px] shrink-0">
                    <div className={`rounded-2xl ${col.bg} border ${col.border} px-3 py-3 mb-3`}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs font-black text-gray-500 bg-gray-100 rounded-full px-2 py-0.5 border border-gray-200">
                          {colContacts.length}
                        </span>
                      </div>
                    ...
                    <div className="flex flex-col gap-2 flex-1">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 rounded-2xl border border-dashed border-gray-200 px-3 py-2.5 text-xs font-bold text-gray-400 hover:border-indigo-400/40 hover:text-indigo-400 transition"
                      >
                        <Plus size={12} /> ����
                      </button>
                    </div>
              ...
              <div className="flex flex-wrap items-center gap-2 border-b border-gray-100 px-5 py-4">
                <div className="relative flex-1 min-w-[160px]">
                  ...
                  <input
                    ...
                    placeholder="����� ��, ������, �����..."
                    className="w-full rounded-xl border border-gray-200 bg-gray-50 py-2 ps-8 pe-3 text-xs text-gray-900 placeholder:text-gray-400 outline-none focus:border-indigo-500/50"
                  />
                </div>
                <div className="relative">
                  <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="rounded-xl border border-gray-200 bg-white text-gray-600 py-2 ps-3 pe-7 text-xs appearance-none outline-none focus:border-indigo-500/50">
                    <option value="">�� ��������</option>
                    {STATUS_COLUMNS.map(s => <option key={s.key} value={s.key}>{s.label}</option>)}
                  </select>
                ...
                <div className="relative">
                  <select value={filterProject} onChange={e => setFilterProject(e.target.value)} className="rounded-xl border border-gray-200 bg-white text-gray-600 py-2 ps-3 pe-7 text-xs appearance-none outline-none focus:border-indigo-500/50">
                    <option value="">�� ���������</option>
                ...
                {(search || filterStatus || filterProject) && (
                  <button type="button" onClick={() => { setSearch(""); setFilterStatus(""); setFilterProject(""); }} className="rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-xs font-bold text-indigo-400 hover:bg-gray-100">
                    ���
                  </button>
                )}
              ...
              <div className="overflow-x-auto">
                    ...
                    {filteredContacts.map(c => {
                      ...
                      return (
                        <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-4 py-3 text-xs text-gray-400 font-mono" dir="ltr">{c.phone ?? "�"}</td>
                          <td className="px-4 py-3 text-xs text-gray-400 truncate max-w-[140px]" dir="ltr">{c.email ?? "�"}</td>
                          <td className="px-4 py-3 text-xs font-bold text-emerald-700">{c.value != null ? fmtMoney(c.value) : "�"}</td>
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1">
                              ...
                              {c.status === "CLOSED_WON" && (
                                <Link
                                  href={`/dashboard/erp/invoice?client=${encodeURIComponent(c.name)}&contactId=${c.id}`}
                                  className="inline-flex items-center gap-1 rounded-lg border border-indigo-500/20 bg-indigo-500/[0.08] px-2.5 py-1.5 text-xs font-bold text-indigo-700 hover:bg-indigo-500/15 transition"
                                >
                                  <ReceiptText size={12} /> �������
                                </Link>
                              )}
                ...
                {filteredContacts.length === 0 && (
                  <div className="flex flex-col items-center justify-center py-16 text-gray-400">
                    ...
                    <p className="font-bold">{search || filterStatus || filterProject ? "��� ������" : "��� ������ �����"}</p>
                    <button
                      type="button"
                      onClick={() => setModal({ mode: "add" })}
          ...
          {view === "projects" && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <p className="text-sm font-bold text-gray-500">{projects.length} ��������</p>
                <button
                  type="button"
                  onClick={() => setAddProjOpen(v => !v)}
                  className="flex items-center gap-2 rounded-xl border border-indigo-500/20 bg-indigo-500/[0.08] px-4 py-2 text-xs font-bold text-indigo-700 hover:bg-indigo-500/15 transition"
                >
                  <FolderPlus size={13} /> ������ ���
                </button>
              </div>
              ...
              {addProjOpen && (
                <div className="rounded-2xl border border-gray-200 bg-white p-5 space-y-3">
                  ...
                  {projErr && <p className="text-xs text-rose-700">{projErr}</p>}
                  <input value={projName} onChange={e => setProjName(e.target.value)} placeholder="�� ������" className={inputCls} />
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[10px] font-bold text-gray-400 mb-1">�����</label>
                  ...
                  <label className="flex items-center gap-2 text-xs font-bold text-gray-600 cursor-pointer">
                    <input type="checkbox" checked={projActive} onChange={e => setProjActive(e.target.checked)} className="rounded border-gray-200 accent-indigo-500" />
                    ������ ����
                  </label>
                  <div className="flex gap-2">
                    <button type="button" onClick={saveProject} disabled={savingProj} className="flex items-center gap-2 rounded-xl bg-indigo-500 px-4 py-2 text-xs font-bold text-white disabled:opacity-50">
                      {savingProj ? <Loader2 size={13} className="animate-spin" /> : <Plus size={13} />} ����
                    </button>
                    <button type="button" onClick={() => setAddProjOpen(false)} className="rounded-xl border border-gray-200 bg-gray-50 px-4 py-2 text-xs font-bold text-gray-500">
                      �����
                    </button>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {projects.map(p => {
                  ...
                  return (
                    <div key={p.id} className={`rounded-2xl border bg-white p-5 ${p.isActive ? "border-gray-200" : "border-gray-100 opacity-60"}`}>
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <span className={`text-[10px] font-bold rounded-full px-2 py-0.5 ${p.isActive ? "bg-emerald-500/[0.12] text-emerald-700" : "bg-gray-100 text-gray-400"}`}>
                              {p.isActive ? "����" : "������"}
                            </span>
                          </div>
                        </div>
                        <button
                          ...
                          className="rounded-lg p-1.5 text-gray-300 hover:text-rose-400 hover:bg-rose-500/[0.10] transition"
                        >
                          <Trash2 size={13} />
                        </button>
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        <p className="text-xs text-gray-400">
                          <strong className="text-gray-700">{pContacts.length}</strong> ������
                        </p>
                        {pValue > 0 && <p className="text-xs font-black text-emerald-700">{fmtMoney(pValue)}</p>}
                      </div>
                      {pContacts.length > 0 && (
          ...
          {organizations.length > 0 && (
            <section className="mt-8 space-y-4">
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-full border border-indigo-500/20 bg-indigo-500/[0.10] px-3 py-0.5 text-xs font-bold text-indigo-700">����� �����</span>
              </div>
              <CrmOrganizationsAdminTable
                organizations={organizations}
                showUnifiedBillingLinks={showUnifiedBillingLinks}
